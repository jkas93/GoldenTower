...
async function seedDatabase() {
  console.log(
    '🌱 Iniciando población de base de datos con datos de muestra...\n',
  );
  ...
  try {
    ...
    for (const emp of sampleEmployees) {
      ...
      if (emailVerified) {
        ...
        console.log(
          `   └─ Ficha laboral creada con salario S/ ${salary.toFixed(2)}`,
        );
      } else {
    ...
    console.log(
      `   ├─ Con ficha laboral: ${sampleEmployees.filter((e) => e.emailVerified).length}`,
    );
    console.log(
      `   └─ Pendientes: ${sampleEmployees.filter((e) => !e.emailVerified).length}`,
    );
    console.log(
