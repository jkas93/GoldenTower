  ...
  async onModuleInit() {
    ...
    try {
      ...
      const privateKey = this.configService
        .get<string>('ADMIN_PRIVATE_KEY')
        ?.replace(/\\n/g, '\n');
      const clientEmail = this.configService.get<string>('ADMIN_CLIENT_EMAIL');

      if (!projectId || !privateKey || !clientEmail) {
        throw new Error(
          'Faltan credenciales de Firebase. Verifica las variables de entorno: ADMIN_PROJECT_ID, ADMIN_PRIVATE_KEY, ADMIN_CLIENT_EMAIL',
        );
      }
      ...
      if (!privateKey.includes('BEGIN PRIVATE KEY')) {
        throw new Error(
          'ADMIN_PRIVATE_KEY tiene formato inválido. Debe incluir "-----BEGIN PRIVATE KEY-----"',
        );
      }
      ...
      const initPromise = new Promise<void>((resolve, reject) => {
        ...
        try {
          if (!admin.apps.length) {
            ...
            this.logger.log(
              'ℹ️ Firebase App ya estaba inicializada, reutilizando instancia',
            );
          }
      ...
      this.logger.log(
        `✅ Firebase inicializado correctamente en ${this.initializationTime}ms`,
      );
      this.logger.log(`📊 Project ID: ${projectId}`);
    } catch (error) {
      this.initializationTime = Date.now() - startTime;
      this.logger.error(
        `❌ Error al inicializar Firebase (${this.initializationTime}ms):`,
        error.message,
      );
