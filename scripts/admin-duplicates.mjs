/**
 * Script administrativo para anÃ¡lisis y limpieza de duplicados
...
async function analyzeDuplicates() {
    console.log('ðŸ” Analizando empleados en la base de datos...\n');

    const snapshot = await firestore.collection('employees').get();
    const employees = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));

    console.log(`ðŸ“Š Total de empleados: ${employees.length}\n`);
    ...
    console.log('ðŸ”´ DNIs DUPLICADOS:');
    ...
    for (const [dni, list] of dniMap.entries()) {
        if (list.length > 1) {
            ...
            console.log(`\n${dni} (${list.length} registros - se eliminarÃ¡n ${list.length - 1}):`);

            // Ordenar por createdAt para mostrar cuÃ¡l se mantendrÃ­a
            list.sort((a, b) => (b.createdAt || '').localeCompare(a.createdAt || ''));

            list.forEach((emp, index) => {
                const status = index === 0 ? 'âœ… MANTENER' : 'âŒ ELIMINAR';
    ...
    if (dniDuplicates === 0) {
        ...
        console.log(`âš ï¸  Registros a eliminar: ${totalDniRecordsToDelete}`);
    }

    // Analizar Emails duplicados
    console.log('\n\nðŸ”´ EMAILS DUPLICADOS:');
    console.log('â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”â”');
    ...
    for (const [email, list] of emailMap.entries()) {
        if (list.length > 1) {
            ...
            console.log(`\n${email} (${list.length} registros - se eliminarÃ¡n ${list.length - 1}):`);
    ...
    if (emailDuplicates === 0) {
        ...
        console.log(`\nâš ï¸  Registros a eliminar: ${totalEmailRecordsToDelete}`);
    }
...
async function cleanupDuplicates() {
    console.log('\nðŸ§¹ Ejecutando limpieza de duplicados...\n');
        ...
        if (list.length > 1) {
            ...
            for (const emp of toDelete) {
                if (!deletedIds.has(emp.id)) {
                    ...
                    console.log(`ðŸ—‘ï¸  DNI: Eliminado ${emp.dni} (${emp.name}) - ID: ${emp.id}`);
                }
            }
        }
    }
    ...
    for (const [email, list] of emailMap.entries()) {
        ...
        if (activeList.length > 1) {
            ...
            for (const emp of toDelete) {
                ...
                console.log(`ðŸ—‘ï¸  EMAIL: Eliminado ${emp.email} (${emp.name}) - ID: ${emp.id}`);
            }
        }
    }
...
(async () => {
    try {
        ...
        if (analysis.hasDuplicates) {
            console.log('\n\nâš ï¸  Â¿Deseas ejecutar la limpieza? (Esta acciÃ³n es IRREVERSIBLE)');
            ...
            if (process.argv.includes('--clean')) {
                console.log('\nâš ï¸  INICIANDO LIMPIEZA EN 3 SEGUNDOS...');
                await new Promise(resolve => setTimeout(resolve, 3000));
                await cleanupDuplicates();
            }
        } else {
            console.log('\nâœ… La base de datos estÃ¡ limpia, no hay duplicados.');
        }
